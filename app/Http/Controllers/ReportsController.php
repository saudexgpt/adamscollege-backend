...
class ReportsController extends Controller
{
    ...
    public function financialReport()
    {
        ...
        $month_str = date('F', strtotime('now'));
        $month_int = date('m', strtotime('now'));
        ...
        $income_n_expenses = IncomeAndExpense::groupBy( 'pay_month', 'status')
            ->where('deletable', '0')
            ->where(['school_id' => $school_id, 'pay_year' => $year])            
            ->selectRaw('SUM(amount) as sum, status, date, pay_month')
            ->get();
        ...
        foreach ($income_n_expenses as $income_n_expense) {
            $month_int = date('m', strtotime($income_n_expense->date));
            $sum = (double) $income_n_expense->sum;
            if ($income_n_expense->status === 'income') {
                $income_data[$month_int - 1] = $sum;
                $total_income += $sum;
            }
            if ($income_n_expense->status === 'expenses') {
                $expenses_data[$month_int - 1] = $sum;
        ...
        return $this->render(compact('bar_chart_series', 'categories', 'pie_chart_series', 'profit', 'total_income', 'total_expenses'));
    }
