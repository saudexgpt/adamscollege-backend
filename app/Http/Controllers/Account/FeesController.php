...
class FeesController extends Controller
{
    ...
    public function storeFeeSettings(Request $request)
    {
        ...
        foreach ($level_ids as $level_id) :
            $school_fee = SchoolFee::where([ 'level_id' => $level_id, 'purpose' => $purpose])->first();
    ...
    {
        $school_id = 1; // $this->getSchool();
        $fee = School::find($school_id)->registration_fee;
        $paystack_key = School::find($school_id)->paystack_key;

        return response()->json(compact('fee', 'paystack_key'), 200);
    }
