...
class PaymentsController extends Controller
{
    ...
    public function fetchRegistrationFeePayments(Request $request)
    {
        ...
        $fee_payments = RegistrationFeePayment::with(
            'student.user'
        )
            ->where([
                'school_id' => $school_id,
        ...
        return response()->json(compact('fee_payments'), 200);
    }
    public function paymentsMonitorTable(Request $request)
    {
        ...
        return $this->render(compact('payment_monitors'));
    }
    public function studentPaymentTable(Request $request)
    {
        $this->applySpecificStudentSchoolFees($request);
        $school_id = $this->getSchool()->id;
    ...
    public function applySpecificStudentSchoolFees(Request $request)
    {
        ...
        $level_id = $student->current_level;
            ...
            $school_fees = SchoolFee::groupBy('level_id')
    ...
    public function payViaCardForRegistration(Request $request)
    {
        $user = $this->getUser();
        $school_id = 1; // $this->getSchool()->id;
